...
const sendMessage = (chatId, address, name, {timestamp, transactionHash, token, value, valueInUSD, incoming}) => {
...
const handleOne = ({address, name, chatId, lastTs}) => {
    return getNewTransfers(address, lastTs).then(transfers => {
        ...
        promises.push(admin.database().ref(`/watch/${chatId}/${address}/lastTs`).set(transfers[0].timestamp));
